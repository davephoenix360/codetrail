...
| 2026-06-12 | **Multi-account DEFERRED to v2.0+** | Diepreye | Phase 2.5 was built (8 commits e5126b7→fd53639) but auth edge cases (post-signin uid ordering, public-lookup index, public-read security tradeoff) consumed all dev time and starved the actual product. MVP collapses to single GitHub account. Multi-account code is preserved in git history and the `oauth-with-linked-accounts` skill; we'll revisit when the foundation (streak, hype-man features) is solid. |
